    ...
    class EncryptWord
    {
        ...
        public string encrypt(String x) {
            ...
            for (int i = 0; i < x.Length; i++) {
                ...
                if (x[i] >= UP_LOW && x[i] <= UP_HIGH)
                    xEnd += (char)(((int)(x[i])+SHIFT-UP_LOW)%LETTERS +UP_LOW);
                //if not upper, check if lower and shift to another lowercase value.
                else if (x[i] >= LOW_LOW && x[i] <= LOW_HIGH)
                    xEnd += (char)(((int)(x[i])+SHIFT-LOW_LOW)%LETTERS +LOW_LOW);
        ...
        public string decrypt(string x)
        {
            ...
            for (int i = 0; i < x.Length; i++) {
                ...
                //also uppercase if it is.
                if (x[i] >= UP_LOW && x[i] <= UP_HIGH)
                            xEnd += (char)(((int)(x[i]) + (LETTERS - SHIFT) - UP_LOW) % LETTERS + UP_LOW);
                            //if not upper, check if lower and shift to another lowercase value.
                        else if (x[i] >= LOW_LOW && x[i] <= LOW_HIGH)
                            xEnd += (char)(((int)(x[i]) + (LETTERS - SHIFT) - LOW_LOW) % LETTERS + LOW_LOW);
